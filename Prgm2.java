...
public class Prgm2
{
	...
	long sortHoare(int arr[], int low, int high)
	{
		long startTime = System.currentTimeMillis();
		if (low < high) {
			int i = partitionHoare(arr, low, high);
			
			sortHoare(arr, low, i - 1);
			sortHoare(arr, i+1, high);
		}
		return System.currentTimeMillis() - startTime;
	}
	...
	int partitionHoare(int arr[], int low, int high)
	{
		...
		while (arr[i] < x) // finds val > pivot on left
		{
			i++;
		}
		
		while (arr[j] > x) // finds val < pivot on right
		{
			j--;
		}
		
		if (i < j) // swaps if i & j are still in their respective sides
		{
			int temp = arr[i];
			arr[i] = arr[j];
			arr[j] = temp;
	...
	* stop, no need to keep sorting, just return. */
	...
	public static void main(String args[])
	{
		...
		System.out.println("sorted same values array, time: " + Long.toString(sameTime)); */
		arr = getAscendingArray(NUM_ITEMS);
		long ascTime = ob.bubbleSort(arr, false);
		arr = getDescendingArray(NUM_ITEMS);
		long descTime = ob.bubbleSort(arr, false);
		arr = getRandomArray(NUM_ITEMS);
		long randomTime = ob.bubbleSort(arr, false);
		arr = getSameArray(NUM_ITEMS);
		long sameTime = ob.bubbleSort(arr, false);
